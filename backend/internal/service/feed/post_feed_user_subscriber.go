...
func (s *PostFeedUserSubscriber) prepareConsumer(ctx context.Context, userId model.UserId) (queue.QueueConsumer, error) {
	var err error
	...
	data, err := consumer.ReceiveData(ctx)
	if err != nil {
		if err := consumer.Stop(ctx); err != nil {
			logger.Error(ctx, err, "failed stopping consumer")
		}
		return nil, err
	}
	...
	go func() {
		...
		for d := range data {
			...
			if err != nil {
				logger.Error(ctx, err, "failed unmarshalling notification", "notification", string(d))
			} else {
	...
	logger.Debug(ctx, "consumer created", "userId", userId)
	return consumer, nil
}
